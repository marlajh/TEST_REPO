...
This script is a YTD version of the original Crisis script, 
getting all kcids yielded by running the report for each Monday of YTD.

The code produces two output datasets:

The first output dataset contains all kcids with 3 or more crisis programs, 
or IP, or IT auths during the last 30 days as of each Monday of YTD interval

Put first output dataset on first tab of report excel file.
Label the tab: #first_tab___3plus_crisis_IP_or_IT_past_30days

The second output dataset contains  all kcids with 2 or more crisis programs
 or 5 or more IT auths during the last 30 days as of each Monday of YTD interval
Put second output dataset on first tab of report excel file.

Can label first tab as:  #first_tab___3plus_crisis_IP_or_IT_past_30days
      second tab as: #second_tab___2plusIT_or_5plus_crisis_past_30days
...
declare @date_30days_prior_to_rundate date,
		...
		@baseline_date date,
		@first_monday_2022 date,
		@days_to_add integer

drop table if exists  #kcids_from_2022_for_1st_tab 
create table #kcids_from_2022_for_1st_tab ( kcid integer
                                            ,MCO char(14)
											)

drop table if exists  #kcids_from_2022_for_2nd_tab 
create table #kcids_from_2022_for_2nd_tab ( kcid integer, MCO char(12) )

drop table if exists  #WeeklyRundate_30DaysPriorDate_90DaysPriorDate
create table #WeeklyRundate_30DaysPriorDate_90DaysPriorDate (
Loop_Number integer
,WeeklyRundate date
,_30DaysPriorDate date
,_90DaysPriorDate date
)
...
create table #first_tab_of_final_file___3plus_crisis_IP_or_IT_past_30days (
 loop_number integer
, baseline_date date
, date_30days_prior_to_rundate  date
, date_90days_prior_to_rundate date
, kcid integer
...
set @first_monday_2022= DATEFROMPARTS ( 2022, 1, 3) 

set @loop_number  = 0
set @baseline_date = getdate()

WHILE ( @loop_number <= 51 and @baseline_date <= getdate() ) 
...
set @days_to_add = @loop_number * 7

 set @baseline_date = dateadd (dd,  @days_to_add , @first_monday_2022 ) 
 
 set  @date_30days_prior_to_rundate  = dateadd(dd,-30,@baseline_date ) 
 set  @date_90days_prior_to_rundate  = dateadd(dd,-90,@baseline_date ) 


 /*
if ( @loop_number  = 2) set @baseline_date = DATEFROMPARTS ( 2021, 2, 1 )  
if ( @loop_number  = 3) set @baseline_date = DATEFROMPARTS ( 2021, 3, 1 )  
if ( @loop_number  = 4) set @baseline_date = DATEFROMPARTS ( 2021, 4, 1 )  
if ( @loop_number  = 5) set @baseline_date = DATEFROMPARTS ( 2021, 5, 1 )  
if ( @loop_number  = 6) set @baseline_date = DATEFROMPARTS ( 2021, 6, 1 )  
if ( @loop_number  = 7) set @baseline_date = DATEFROMPARTS ( 2021, 7, 1 )  
if ( @loop_number  = 8) set @baseline_date = DATEFROMPARTS ( 2021, 8, 1 )  
if ( @loop_number  = 9) set @baseline_date = DATEFROMPARTS ( 2021, 9, 1 )  
if ( @loop_number  = 10) set @baseline_date = DATEFROMPARTS ( 2021, 10, 1 )  
if ( @loop_number  = 11) set @baseline_date = DATEFROMPARTS ( 2021, 11, 1 )  
if ( @loop_number  = 12) set @baseline_date = DATEFROMPARTS ( 2021, 12, 1 )  

*/



insert into  #WeeklyRundate_30DaysPriorDate_90DaysPriorDate  values (
  @loop_number 
, @baseline_date 
, @date_30days_prior_to_rundate  
, @date_90days_prior_to_rundate
)
...
insert into  #MCO_data
 select 
   ...
   /* Despite 'select distinct', due to all the left joins, a person will be counted once for each instance of any relevant program authorizaiton  */
 insert into #first_tab_of_final_file___3plus_crisis_IP_or_IT_past_30days
	...
	*/
	...
	insert into #kcids_from_2022_for_1st_tab
	select distinct kcid, MCO
	from #first_tab_of_final_file___3plus_crisis_IP_or_IT_past_30days

    insert into  #kcids_from_2022_for_2nd_tab 
	select distinct kcid, MCO
	from #second_tab_of_final_file___2plusIT_or_5plus_crisis_past_30days
	...
	select distinct kcid, MCO from #kcids_from_2022_for_1st_tab

	--select distinct kcid from #kcids_from_2022_for_1st_tab

	select distinct kcid, MCO from #kcids_from_2022_for_2nd_tab 

    --select distinct kcid from #kcids_from_2022_for_2nd_tab 
/*
	select loop_number
	, baseline_date 
	, date_30days_prior_to_rundate  
	, date_90days_prior_to_rundate 
	, count(distinct kcid)  count_distinct_kcids
	from #first_tab_of_final_file___3plus_crisis_IP_or_IT_past_30days
	group by 
	loop_number
	, baseline_date 
	...
	select * from #WeeklyRundate_30DaysPriorDate_90DaysPriorDate
